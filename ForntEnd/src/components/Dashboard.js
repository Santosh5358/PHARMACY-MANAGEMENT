...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      try {
        const response = await axios.get('https://pharmacy-management-7t8g.onrender.com/items');
  ...
  const handleRemoveItemDB = async (itemId) => {
    try {
      await axios.delete(`https://pharmacy-management-7t8g.onrender.com/items/${itemId}`);
  ...
  const handleAddItem = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://pharmacy-management-7t8g.onrender.com/items', newItem);
  ...
  const handleCheckout = async () => {
    ...
    try {
      // Send checkout data to the backend
       await axios.post('https://pharmacy-management-7t8g.onrender.com/api/checkout', checkoutData);
  ...
  const handleUpdateItem = async () => {
    try {
      await axios.put(`https://pharmacy-management-7t8g.onrender.com/items/${editItem._id}`, editItem);
